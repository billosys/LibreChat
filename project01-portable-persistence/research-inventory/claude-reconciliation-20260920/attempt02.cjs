...
function messageDifferences(expected, stored, parentId, scope) {
  const checks = {
    ...
    isEditedAbsent: !present(stored, 'isEdited'),
  ...
  try {
    ...
    for (const expected of projection) {
      ...
      for (const [i, e] of expected.messages.entries()) {
        ...
        if (!negativeControls) {
          assert.equal(failed.length, 0, 'Positive comparator control failed');
          const mutations = {
            ...
            unfinished: { unfinished: true }, isEditedAbsent: { isEdited: true }, error: { error: true },
          };
