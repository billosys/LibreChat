...
Status: **initial research and draft design prepared; no application implementation or runtime validation performed**. The Operator has named SQLite as the target for the initial Claude corpus; the staged Rust-service architecture remains a proposal.
...
The current workflow is one-contributor investigation, selected by the Operator on 2026-09-20. The next action is focused Arc01 preparation: map the pilot's complete call graph and side effects, characterize its behavior, run the relevant existing Mongo tests, and settle concrete contract and acceptance details. We are ready for this investigation; the conversion is not implementation-ready. The intended later progression is two contributors for deeper work and three once implementation is well defined. See the project plan for transition conditions and the unchanged acceptance requirements. Claude is the initial dataset, to be fully accounted for in SQLite before a subsequent OpenAI/GPT import. The export location and initial import route remain pending; source characterization can proceed.
